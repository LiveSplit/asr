...
impl State {
    ...
    fn sms_plus(&self, game: &Process, is_64_bit: bool, core_name: &str) -> Option<Address> {
        ...
        Some(if is_64_bit {
            ...
            let ptr = SIG.scan_once(game, (self.core_base, module_size))? + 5;
            ptr + 0x8 + game.read::<i32>(ptr).ok()?
        } else {
            const SIG: Signature<4> = Signature::new("83 FA 02 B8");
            let ptr = SIG.scan_once(game, (self.core_base, module_size))? + 4;
            game.read::<Address32>(ptr).ok()?.into()
        })
    }
    ...
    fn gearsystem(&self, game: &Process, is_64_bit: bool, core_name: &str) -> Option<Address> {
        ...
        Some(if is_64_bit {
            ...
            let ptr = SIG.scan_once(game, (self.core_base, module_size))? + 8;
            let offset = game
            ...
            let ptr = SIG.scan_once(game, (self.core_base, module_size))? + 7;
            let offset = game
