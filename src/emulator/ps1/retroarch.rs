    ...
    pub fn find_ram(&mut self, game: &Process) -> Option<Address> {
        ...
        if core_name == SUPPORTED_CORES[0] || core_name == SUPPORTED_CORES[1] {
            ...
            if is_64_bit {
                ...
                let ptr = SIG.scan(game, core_address, game.get_module_size(core_name).ok()?)? + 3;
                ...
                let ptr = SIG.scan(game, core_address, game.get_module_size(core_name).ok()?)? + 1;
            ...
            if is_64_bit {
                ...
                let addr = SIG.scan(game, core_address, game.get_module_size(core_name).ok()?)? + 3;
            ...
            if is_64_bit {
                ...
                let ptr = SIG.scan(game, core_address, game.get_module_size(core_name).ok()?)? + 9;
                let ptr = game.read::<Address32>(ptr).ok()?;
                Some(game.read::<Address32>(ptr).ok()?.into())
            }
        } else {
