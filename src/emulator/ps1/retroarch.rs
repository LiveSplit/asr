        ...
        match core_name {
            "mednafen_psx_hw_libretro.dll" | "mednafen_psx_libretro.dll" => {
                match pe::MachineType::read(game, core_address) {
                    ...
                    Some(_) => {
                        const SIG: Signature<11> =
                            Signature::new("A1 ?? ?? ?? ?? 81 E3 FF FF 1F 00");
                        let ptr = SIG.scan_process_range(
                            game,
                            (
                                core_address,
            ...
            "swanstation_libretro.dll" => match pe::MachineType::read(game, core_address) {
                Some(MachineType::X86_64) => {
            ...
            "pcsx_rearmed_libretro.dll" => match pe::MachineType::read(game, core_address) {
                Some(MachineType::X86_64) => {
