use crate::{
    file_format::pe, signature::Signature, Address, Address32, Process, Address64,
};
...
impl State {
    pub fn find_ram(&mut self, game: &Process) -> Option<Address> {
        ...
        let is_64_bit = pe::MachineType::read(game, main_module_address) == Some(pe::MachineType::X86_64);
        ...
        if core_name == SUPPORTED_CORES[0] || core_name == SUPPORTED_CORES[1] {
            ...
            if is_64_bit {
                ...
                let ptr = SIG.scan_process_range(game, (core_address, game.get_module_size(core_name).ok()?))? + 3;
                ...
                let ptr = SIG.scan_process_range(game, (core_address, game.get_module_size(core_name).ok()?))? + 1;
            ...
            if is_64_bit {
                ...
                let addr = SIG.scan_process_range(game, (core_address, game.get_module_size(core_name).ok()?))? + 3;
            ...
            if is_64_bit {
                ...
                let ptr = SIG.scan_process_range(game, (core_address, game.get_module_size(core_name).ok()?))? + 9;
                let ptr = game.read::<Address32>(ptr).ok()?;
                Some(game.read::<Address32>(ptr).ok()?.into())
            }        
        } else {
