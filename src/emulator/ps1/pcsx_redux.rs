use crate::{Address, signature::Signature, MemoryRangeFlags, file_format::pe, Process, Address64, Address32};
...
impl State {
    pub fn find_ram(&mut self, game: &Process) -> Option<Address> {
        ...
        self.is_64_bit = pe::MachineType::read(game, main_module_range.0) == Some(pe::MachineType::X86_64);
        ...
        if self.is_64_bit {
            ...
            self.addr_base = game.memory_ranges()
                .filter(|m| m.flags().unwrap_or_default().contains(MemoryRangeFlags::WRITE))
                .find_map(|m| SIG.scan_process_range(game, (m.address().unwrap_or_default(), m.size().unwrap_or_default())))?
                + 2;
    ...
    pub fn keep_alive(&self, game: &Process) -> bool {
        if self.is_64_bit {
            let Some(addr) = game.read::<Address64>(self.addr_base).ok() else { return false };
            self.addr == addr.into()
        } else {
            let Some(addr) = game.read::<Address32>(self.addr_base).ok() else { return false };
            self.addr == addr.into()
        }
    }
